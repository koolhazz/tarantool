...
struct TupleHash
{
						 ...
						 const struct key_def *key_def)
	{
		...
		TupleFieldHash<TYPE, MORE_TYPES...>::hash(&field, key_def,
				0, &h, &carry, &total_size);
		return PMurHash32_Result(h, carry, total_size);
	}
};
...
{
	for (uint32_t k = 0; k < sizeof(key_hash_arr) / sizeof(key_hash_arr[0]); k++) {
		uint32_t i = 0;
		for (; i < def->part_count; i++)
			    if (def->parts[i].type != key_hash_arr[k].p[i])
				break;
		if (i == def->part_count && key_hash_arr[k].p[i] == UINT32_MAX)
			return key_hash_arr[k].f;
	}
	return key_hash_slow_path;
}
...
tuple_hash_t
tuple_hash_create(const struct key_def *key_def) {
	for (uint32_t k = 0; k < sizeof(tuple_hash_arr) / sizeof(tuple_hash_arr[0]); k++) {
		...
		if (i == key_def->part_count && tuple_hash_arr[k].p[i] == UINT32_MAX)
			return tuple_hash_arr[k].f;
	}

	return tuple_hash_slow_path;
}
